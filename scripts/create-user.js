 ...
 * Installation & Usage:
 *   1. npm install
 *   2. node scripts/create-user.js [--interactive] [--email EMAIL --username USERNAME --password PASSWORD]
 * 
 * Examples:
 *   node scripts/create-user.js --interactive
 *   node scripts/create-user.js --email test@example.com --username TestUser --password Pass123!
 */
...
const API_KEY = process.env.FIREBASE_API_KEY || 'AIzaSyBmFJkfBJrCJpBvWL1SmUz9EW9bW4VQQm8'; // Web API key
...
async function main() {
  try {
    ...
    await httpsRequest(
      `${FIRESTORE_URL}/userStates/${uid}?key=${API_KEY}`,
      'PATCH',
      { fields: userStateData }
    );
