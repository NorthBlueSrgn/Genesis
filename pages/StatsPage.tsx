...
import SubstatProgressionGuide from '../components/SubstatProgressionGuide';
...
import { Lock, Sparkles, Wand2, Map as MapIcon, Wind, Link as LinkIcon, TrendingUp, Target, ChevronDown, Activity, Percent, ChevronsUp, Trophy, UserMinus, AlertTriangle, Scaling, Calculator, Scale, BookOpen } from 'lucide-react';
...
const LockedDisplay: React.FC<{ requiredRank: string }> = ({ requiredRank }) => (
    <div className="bg-gray-800 p-4 rounded-lg text-center h-full flex flex-col justify-center border border-gray-700">
...
const ResonanceSignatureCard: React.FC<{ profile: ResonanceSignature; rank: Rank; }> = ({ profile, rank }) => {
    ...
    const currentRankIndex = rank ? RANKS.findIndex(r => r.name === rank.name) : -1;
    const abilityUnlocked = currentRankIndex >= 0 && currentRankIndex >= RANKS.findIndex(r => r.name === 'C');
    const domainUnlocked = currentRankIndex >= 0 && currentRankIndex >= RANKS.findIndex(r => r.name === 'A');
    ...
    return (
        <Card className="!border-purple-600/30">
            ...
            <div className="text-left space-y-4 mb-6">
                <div className="bg-gray-800 p-3 rounded-sm border border-purple-900/20">
...
const SubStatProgress: React.FC<{ subStat: SubStat; color: string }> = ({ subStat, color }) => {
    ...
    return (
        <div className="text-[10px]">
            ...
            <div className="w-full bg-gray-900 rounded-full h-1 border border-gray-700">
...
const AttributeCard: React.FC<{ stat: Stat }> = ({ stat }) => {
    ...
    return (
        <Card className="flex flex-col !bg-black/60 hover:border-purple-500/40 transition-all group border-purple-500/20 shadow-[0_0_10px_rgba(168,85,247,0.1)]">
            ...
            <div className="w-full bg-gray-900/40 rounded-full h-1.5 my-4 border border-gray-700/50">
            ...
            {isExpanded && (
             ...
             <button onClick={() => setIsExpanded(!isExpanded)} className="mt-4 pt-2 text-[9px] font-black uppercase text-gray-600 hover:text-purple-300 flex items-center justify-center gap-1 w-full border-t border-purple-500/10 transition-colors">
...
const GrowthVelocityChart: React.FC<{ history: StatSnapshot[] }> = ({ history }) => {
    ...
    return (
        <Card className="!bg-black/60 border-purple-500/30 shadow-[0_0_10px_rgba(168,85,247,0.1)]">
            ...
            <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={data}>
                        ...
                        <XAxis dataKey="date" stroke="#666" fontSize={10} fontStyle="bold" />
                        <YAxis stroke="#666" fontSize={10} domain={['auto', 'auto']} />
                        <Tooltip contentStyle={{ backgroundColor: '#000', border: '1px solid #a855f7', fontSize: '10px' }} />
...
const StatsPage: React.FC = () => {
    ...
    const chartData = useMemo(() => {
        ...
        const forsakenStats = forsaken?.stats || [];
        const forsakenPercentiles = convertStatsToPercentiles(forsakenStats.map(s => ({...s, rank: AttributeRankName.E, subStats: [], lastIncreased: ''}))) || {};
        const statNameValues = Object.values(StatName) || [];
        return statNameValues.map(statName => ({ subject: statName, User: userPercentiles[statName] || 0, Forsaken: forsakenPercentiles[statName] || 0 }));
    ...
    return (
        <div>
            <DossierHeader title="BIOMETRIC ANALYSIS" />
             ...
             <div className="flex justify-between items-center border-b border-gray-800 mb-10 bg-black/10 px-4 md:px-8">
                <div className="flex">
                ...
                <button 
                    ...
                    className="flex items-center gap-2 px-3 py-2 text-[9px] font-orbitron font-black text-cyan-400 hover:text-cyan-300 uppercase tracking-wider transition-colors"
                    title="View Substat Progression Guide"
                >
            ...
            {activeTab === 'overview' && (
                <div className="space-y-8 animate-in fade-in duration-500">
                    <Card className="!bg-black/60 border-purple-500/30 shadow-[0_0_15px_rgba(168,85,247,0.2)]">
                        <h2 className="text-[10px] font-black text-purple-400 tracking-[0.4em] uppercase mb-8 text-center">Global Percentile Projection</h2>
                         <div style={{ width: '100%', height: 350 }}>
                            <ResponsiveContainer>
                                <RadarChart cx="50%" cy="50%" outerRadius="80%" data={chartData}>
                                    <PolarGrid stroke="#30363d" />
                                    <PolarAngleAxis dataKey="subject" tick={{ fill: '#a78bfa', fontSize: 10, fontWeight: 900 }} />
                                    <PolarRadiusAxis angle={30} domain={[0, 100]} tick={{ fill: '#444' }} hide />
                                    ...
                                    <Legend wrapperStyle={{ fontSize: '10px', fontWeight: 'bold', paddingTop: '20px', color: '#c084fc' }} />
                                </RadarChart>
                            </ResponsiveContainer>
                        </div>
                    </Card>
            ...
            {activeTab === 'resonance' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in zoom-in-95">
                    ...
                    <Card className="!bg-black/60 border-fuchsia-500/40 shadow-[0_0_15px_rgba(236,72,153,0.2)] relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-fuchsia-600/10 blur-3xl -mr-16 -mt-16 rounded-full pointer-events-none" />
                        <div className="relative z-10">
                            <h2 className="text-[10px] font-black text-fuchsia-400 tracking-[0.4em] uppercase mb-6 text-center flex items-center justify-center gap-2">
                                <Sparkles size={14} /> Resonance Wheel Projection
                            </h2>
                            <div className="flex justify-center scale-90 md:scale-100 mb-6">
                                <ResonanceWheel userResonance={resonanceSignature.type} resonanceVector={resonanceVector} />
                            </div>
                            <div className="space-y-3 text-[9px] bg-fuchsia-950/20 p-4 rounded-md border border-fuchsia-900/30">
                                <div className="flex items-start gap-3">
                                    <span className="font-black text-fuchsia-400 uppercase tracking-wider min-w-fit">Frequency:</span>
                                    <span className="text-gray-300 font-mono">{resonanceSignature.type}</span>
                                </div>
                                <div className="flex items-start gap-3">
                                    <span className="font-black text-fuchsia-400 uppercase tracking-wider min-w-fit">Tier:</span>
                                    <span className="text-gray-300 font-mono">Level {resonanceSignature.tier} • Band {resonanceSignature.rankBand}</span>
                                </div>
                                <div className="flex items-start gap-3">
                                    <span className="font-black text-fuchsia-400 uppercase tracking-wider min-w-fit">Status:</span>
                                    <span className={`font-mono font-black ${resonanceSignature.awakened ? 'text-emerald-400' : 'text-gray-500'}`}>
                                        {resonanceSignature.awakened ? 'RESONANCE_ACTIVE' : 'CALIBRATING'}
                                    </span>
                                </div>
                            </div>
                        </div>
            ...
            {showProgressionGuide && (
                <SubstatProgressionGuide isOpen={showProgressionGuide} onClose={() => setShowProgressionGuide(false)} />
            )}
        </div>
    );
};
